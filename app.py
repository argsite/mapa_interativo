...
if uploaded_file:
    ...
    if "Qtd. Visitas Domiciliares" in df.columns:
        col_visitas = "Qtd. Visitas Domiciliares"
        vis = (
            df[col_visitas]
            .fillna("")
            .astype(str)
        ...
        df["teve_visita"] = vis.isin(["1", "2", "2+"])
        pct_visita = df["teve_visita"].mean() * 100
        st.metric("Pessoas com pelo menos 1 visita", f"{pct_visita:.1f}%")
